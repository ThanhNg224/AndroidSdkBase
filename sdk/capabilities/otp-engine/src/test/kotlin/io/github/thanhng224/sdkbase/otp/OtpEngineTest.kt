...
class OtpEngineTest {
    ...
    fun `a wrong code returns to awaiting with one fewer attempt`() = runTest {
        ...
        assertEquals(SdkErrors.OTP_INVALID, engine.state.value.error?.code)
        engine.close()
    }
    ...
    fun `exhausting attempts ends the session`() = runTest {
        ...
        assertEquals(SdkErrors.OTP_ATTEMPTS_EXCEEDED, engine.state.value.error?.code)
        engine.close()
    }
    ...
    fun `resend is refused while the cooldown is still running`() = runTest {
        ...
        assertEquals(1, gateway.requestCount)
        assertEquals(SdkErrors.OTP_RESEND_TOO_SOON, engine.state.value.error?.code)
        engine.close()
    }
