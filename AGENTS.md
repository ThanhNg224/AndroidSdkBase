...
- The host owns networking: features declare a gateway interface; no HTTP client, no DI framework, no `GlobalScope` in `sdk/`.
- Nothing escapes to the host as an exception: return `SdkResult`; map host exceptions to `SdkError`.
...
- No secrets in logs: pass identifiers through `redact()`.
